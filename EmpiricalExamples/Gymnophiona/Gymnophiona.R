...
setwd("/home/jfwalker/Desktop/OutlierSims/EmpiricalExamples/Gymnophiona/")
IhateR4 = read.table("Amphibian_genewise_Avg.txt",sep = "\t",header = TRUE)
a4 = IhateR4[,2]
...
Temp4 = Temp4 +theme(legend.position="none") + ylim(-50,60) + annotate("text", x=498, y=1.31742199999999, label= "X",size=6) 
Temp4 = Temp4 + annotate("text", x=3313, y=54.2767650000001, label= "X",size=6)  +theme(legend.position="none") + geom_abline(intercept = 0, slope = 0)
Temp4 = Temp4 + ggtitle("Gymnophiona ΔGWLL")+ theme(plot.title = element_text(face="bold",hjust=0.5),panel.grid.major = element_blank(), panel.grid.minor = element_blank())
Temp4 = Temp4 + geom_vline(xintercept=3313,linetype="dashed") 
Temp4 
...
Temp3 = Temp3 +theme(legend.position="none") + ylim(-0.2,0.3) + annotate("text", x=3313, y=0.296594344262295, label= "X",size=6)
Temp3 = Temp3 + theme(legend.position="none") + geom_abline(intercept = 0, slope = 0)
Temp3 = Temp3 + ggtitle("Gymnophiona Average ΔSSLL")+ theme(plot.title = element_text(face="bold",hjust=0.5),panel.grid.major = element_blank(), panel.grid.minor = element_blank())
Temp3 = Temp3 + geom_vline(xintercept=3313,linetype="dashed") 
Temp3
