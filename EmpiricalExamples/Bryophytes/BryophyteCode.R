...
setwd("/home/jfwalker/Desktop/OutlierSims/EmpiricalExamples/Bryophytes/")
IhateR4 = read.table("BryophytesAlnAdjustment.txt",sep = "\t",header = TRUE)
a4 = IhateR4[,2]
bipartition4 = a4[1:620]
Temp4 = qplot(1:620,bipartition4,ylab="ΔGWLL",xlab="Gene")+theme_bw()+theme(axis.title.x = element_text(size=20), axis.title.y = element_text(size=20), axis.text.x = element_text(color="black",size = 10), axis.text.y = element_text(color="black",size = 10))
Temp4 = Temp4 +theme(legend.position="none") + ylim(-15,15) + annotate("text", x=255, y=-11.9284839998945, label= "X",size=6) 
Temp4 = Temp4 + annotate("text", x=498, y=1.55752200000001, label= "X",size=6)  +theme(legend.position="none") + geom_abline(intercept = 0, slope = 0)
Temp4 = Temp4 + ggtitle("Bryophyte ΔGWLL")+ theme(plot.title = element_text(face="bold",hjust=0.5),panel.grid.major = element_blank(), panel.grid.minor = element_blank())
Temp4 = Temp4 + geom_vline(xintercept=498,linetype="dashed") + geom_vline(xintercept=255,linetype="dashed") 
Temp4


a3 = IhateR4[,3]
bipartition3 = a3[1:620]
Temp3 = qplot(1:620,bipartition3,ylab="Average ΔSSLL", xlab="Gene")+theme_bw()+theme(axis.title.x = element_text(size=20), axis.title.y = element_text(size=20), axis.text.x = element_text(color="black",size = 10), axis.text.y = element_text(color="black",size = 10))
Temp3 = Temp3 +theme(legend.position="none") + ylim(-0.2,0.2) + annotate("text", x=255, y=-0.0113712907529976, label= "X",size=6) 
Temp3 = Temp3 + annotate("text", x=498, y=0.119809384615385, label= "X",size=6)  +theme(legend.position="none") + geom_abline(intercept = 0, slope = 0)
Temp3 = Temp3 + ggtitle("Bryophyte Average ΔSSLL")+ theme(plot.title = element_text(face="bold",hjust=0.5),panel.grid.major = element_blank(), panel.grid.minor = element_blank())
Temp3 = Temp3 + geom_vline(xintercept=498,linetype="dashed") + geom_vline(xintercept=255,linetype="dashed") 
Temp3
