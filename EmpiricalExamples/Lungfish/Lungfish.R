...
setwd("/home/jfwalker/Desktop/OutlierSims/EmpiricalExamples/NeoAves/")
IhateR4 = read.table("NeoAvesAvg.txt",sep = "\t",header = TRUE)
a4 = IhateR4[,2]
...
Temp4 = Temp4 +theme(legend.position="none") + ylim(-30,100)  
Temp4 = Temp4 + annotate("text", x=3228, y=97.0973749999794, label= "X",size=6)  +theme(legend.position="none") + geom_abline(intercept = 0, slope = 0)
Temp4 = Temp4 + ggtitle("Neoaves ΔGWLL")+ theme(plot.title = element_text(face="bold",hjust=0.5),panel.grid.major = element_blank(), panel.grid.minor = element_blank())
Temp4 = Temp4 + geom_vline(xintercept=3228,linetype="dashed") 
Temp4 
...
Temp3 = Temp3 +theme(legend.position="none") + ylim(-0.1,0.3) + annotate("text", x=3228, y=0.2727454353932, label= "X",size=6)
Temp3 = Temp3 + theme(legend.position="none") + geom_abline(intercept = 0, slope = 0)
Temp3 = Temp3 + ggtitle("Neoaves Average ΔSSLL")+ theme(plot.title = element_text(face="bold",hjust=0.5),panel.grid.major = element_blank(), panel.grid.minor = element_blank())
Temp3 = Temp3 + geom_vline(xintercept=3228,linetype="dashed") 
Temp3
