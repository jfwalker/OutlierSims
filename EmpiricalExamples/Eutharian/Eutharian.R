...
setwd("/home/jfwalker/Desktop/OutlierSims/EmpiricalExamples/Eutharian/")
IhateR4 = read.table("EutharianAvg.txt",sep = "\t",header = TRUE)
a4 = IhateR4[,2]
...
Temp4 = Temp4 +theme(legend.position="none") + ylim(-50,150) + annotate("text", x=498, y=1.31742199999999, label= "X",size=6) 
Temp4 = Temp4 + annotate("text", x=2184, y=126.234664000001, label= "X",size=6)  +theme(legend.position="none") + geom_abline(intercept = 0, slope = 0)
Temp4 = Temp4 + ggtitle("Eutherian ΔGWLL")+ theme(plot.title = element_text(face="bold",hjust=0.5),panel.grid.major = element_blank(), panel.grid.minor = element_blank())
Temp4 = Temp4 + geom_vline(xintercept=2184,linetype="dashed") 
Temp4 
...
Temp3 = Temp3 +theme(legend.position="none") + ylim(-0.1,0.5) + annotate("text", x=2184, y=0.433796096219933, label= "X",size=6)
Temp3 = Temp3 + theme(legend.position="none") + geom_abline(intercept = 0, slope = 0)
Temp3 = Temp3 + ggtitle("Eutherian Average ΔSSLL")+ theme(plot.title = element_text(face="bold",hjust=0.5),panel.grid.major = element_blank(), panel.grid.minor = element_blank())
Temp3 = Temp3 + geom_vline(xintercept=2184,linetype="dashed") 
Temp3
