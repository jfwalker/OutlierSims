...
#Carnivory
################
#Pro_ENSG00000130985	93.7953230001003	0.0901011748319888
setwd("/home/jfwalker/Desktop/OutlierSims/EmpiricalExamples/Turtle/")
IhateR4 = read.table("TurtleAvg.txt",sep = "\t",header = TRUE)
a4 = IhateR4[,2]
...
Temp4 = Temp4 +theme(legend.position="none") + ylim(-70,100)  + annotate("text", x=3865, y=34.591222, label= "X",size=6)
Temp4 = Temp4 + annotate("text", x=898, y=94.6385440000004, label= "X",size=6)  +theme(legend.position="none") + geom_abline(intercept = 0, slope = 0)
Temp4 = Temp4 + ggtitle("Turtles ΔGWLL")+ theme(plot.title = element_text(face="bold",hjust=0.5),panel.grid.major = element_blank(), panel.grid.minor = element_blank())
Temp4 = Temp4 + geom_vline(xintercept=898,linetype="dashed") + geom_vline(xintercept=3865,linetype="dashed")
Temp4 


#Pro_ENSG00000170043	34.591222	0.262054712121212
a3 = IhateR4[,3]
...
Temp3 = Temp3 +theme(legend.position="none") + ylim(-0.3,0.3) + annotate("text", x=898, y=0.207086529540482, label= "X",size=6)
Temp3 = Temp3 + theme(legend.position="none") + geom_abline(intercept = 0, slope = 0) + annotate("text", x=3865, y=0.262054712121212, label= "X",size=6)
Temp3 = Temp3 + ggtitle("Turtles Average ΔSSLL")+ theme(plot.title = element_text(face="bold",hjust=0.5),panel.grid.major = element_blank(), panel.grid.minor = element_blank())
Temp3 = Temp3 + geom_vline(xintercept=898,linetype="dashed") + geom_vline(xintercept=3865,linetype="dashed")
Temp3
